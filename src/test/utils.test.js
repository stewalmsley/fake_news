const { sortArticlesOrComments, addKeysToArticles, addKeysToComments, createCroppedBody, 
  createTopicKey, convertTime } = require('../utils')

describe('sortArticlesOrComments()', () => {
  const obj1 = { title: 'article1', commentCount: 0, votes: 1, created_at: '01/01/2018'}
  const obj2 = { title: 'article2', commentCount: 1, votes: 2, created_at: '01/01/2016'}
  const obj3 = { title: 'article3', commentCount: 2, votes: 0, created_at: '01/01/2017'}
  ...
  it('returns an array of objects ordered by the provided key', () => {
    expect(sortArticlesOrComments(originalArr, "votes")).toEqual(arrSortedByVotes);
    expect(sortArticlesOrComments(originalArr, "commentCount")).toEqual(arrSortedByComments);
    expect(sortArticlesOrComments(originalArr, "created_at")).toEqual(arrSortedByDate);
  });
...
describe('addKeysToArticles()', () => {
  const originalArr = [{ body: 'this is a test article', title: 'test', created_at: '01/01/2018', belongs_to: 'football' }]
  it('adds all relevant keys to array of article objects', () => {
    expect(addKeysToArticles(originalArr)[0].croppedBody).toBe('this is a test article')
...
describe('addKeysToComments()', () => {
  const originalArr = [{ body: 'this is a test comment', created_at: '01/01/2018' }]
  it('adds all relevant keys to array of comment objects', () => {
    expect(addKeysToComments(originalArr)[0].croppedBody).toBe('this is a test comment')
...
describe('createCroppedBody()', () => {
  ...
  it('returns a string unchanged if it has fewer than n words', () => {
    expect(createCroppedBody('this is a test string', 20)).toBe('this is a test string');
  });
});
