...
export const createTopicKey = (article => {
    return article.belongs_to ? article.belongs_to[0].toUpperCase() + article.belongs_to.substring(1): 
    '';
})

export const convertTime = (articleOrComment => {
    return articleOrComment.created_at ? dayjs(articleOrComment.created_at).fromNow() :
    '';
})
